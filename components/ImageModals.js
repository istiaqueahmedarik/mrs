...
function ImageModals({data}) {
  ...
  return (
    ...
    <DrawerTrigger> <div className='cursor-pointer flex flex-col m-3 bg-[#222a3e] w-fit p-1 rounded-md text-white '>
          <Image placeholder="blur" blurDataURL="/iconblur.jpg" quality={100} height={400} width={400} src={imageUrl} alt={data.Title} />
          <h1>{data.Title}</h1>
          <p>{data.description}</p>
          
      </div></DrawerTrigger>
    <DrawerContent className=" bg-transparent border-none w-full">
    <div className='w-full m-auto'><Image placeholder="blur" blurDataURL="/iconblur.jpg" quality={100}Slider className="m-5" data={data.children} /></div>
